    ...
    for (const translation of Object.values(payload.translations)) {
      await client.query(
        `INSERT INTO post_translations
          (post_id, lang, title, slug, excerpt, markdown, html, seo_title, seo_description, seo_keywords, image_alt)
...
async function findCmsPostBySlug({ lang = DEFAULT_LANG, slug }) {
  ...
  const result = await db.query(
    `SELECT p.id, p.status, p.hero_image, p.published_at, p.updated_at,
            t.lang, t.title, t.slug, t.excerpt, t.html, t.seo_title, t.seo_description, t.seo_keywords, t.image_alt
