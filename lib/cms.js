...
async function ensureSchema(db = getPool()) {
  ...
  `);
  await db.query(`ALTER TABLE post_translations ADD COLUMN IF NOT EXISTS target_location TEXT`);
  await db.query(`ALTER TABLE post_translations ADD COLUMN IF NOT EXISTS local_seo_title TEXT`);
  await db.query(`ALTER TABLE post_translations ADD COLUMN IF NOT EXISTS local_seo_description TEXT`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_posts_status_published ON posts(status, published_at DESC)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_post_translations_lang_slug ON post_translations(lang, slug)`);
  schemaReady = true;
  return true;
}
