...
function normalizeDbArticle(row) {
  ...
  const slug = row.slug || slugWithHash(row.title, originalLink || row.link || row.id);
  const published = row.published_at || row.date || row.fetched_at || '';
  return {
    ...row,
    type: 'scraped',
    title: row.title || '',
    ...
    link: `/blog/${slug}`,
    original_link: originalLink,
    source: inferSource(row.source, originalLink || row.link),
...
async function listScrapedArticles({ db = getPool(), source = 'all', offset = 0, pageSize = 20 } = {}) {
  ...
  try {
    ...
    return {
      articles: rows.rows.map(normalizeDbArticle),
...
async function listSitemapArticles({ db = getPool(), limit = 1000 } = {}) {
  ...
  try {
    ...
    return result.rows.map(normalizeDbArticle);
  } catch (error) {
    console.warn('[articles] DB sitemap failed, using static fallback:', error.message);
    return loadStaticArticles().map(normalizeStaticArticle).slice(0, limit);
  }
}
...
async function findScrapedArticleBySlug(slug, { db = getPool() } = {}) {
  ...
  if (db) {
    try {
      ...
      if (rows.length) return toDetailArticle(normalizeDbArticle(rows[0]));
    } catch (error) {
